...
The overlay is the **FMA7163 skin surface from the same BodyParts3D dataset as the spine**, transformed with the identical axis conversion and uniform scale. It replaces the procedural dummy. The surface is clustered for browser performance; source URL, checksum and processing are in `public/anatomy/body-source.json`. After downloading its documented source to `/tmp/spinelab-skin.stl`, run `python3 scripts/pack-body.py` followed by `node scripts/repair-body.mjs` to reproduce the derived mesh. The outer skin is reconstructed at 6 mm and smoothed to close scan holes. External genital detail is omitted from the distributed overlay. The body GLB and its derivatives retain CC BY-SA 2.1 Japan.

Body and disc controls have separate inspector tabs. Camera orbit is the default; Pose enables direct body manipulation, Pan or Shift-drag moves the view, and Fit body / Fit spine frame the current posed geometry. Follow-centering can be disabled and automatically releases on manual pan. Transparent skin uses a nearest-surface depth pass so overlapping source layers do not hide the spine.

`lib/spine-dynamics.js` implements a reduced-order articulated model with joints at all 23 disc levels, regional rotational stiffness, joint limits, inertia, damping, bounded time substeps, and load-dependent compliance. Rigid vertebrae rotate about the sourced disc centers; soft tissues blend adjacent transforms. A generic axial stiffness limits extra-load shortening to 8% of each disc's baseline thickness. Pose targets represent active control. The illustrative gravity moment uses the added load and an upper-trunk lever approximation; it is not a solved muscle-force model. Skinning and limb IK are approximate, especially at extreme poses.

These mechanics are **not clinically calibrated**: no patient-specific material properties, validated muscle recruitment, facet collision solver, finite-element tissue mechanics, or injury prediction are provided. Existing disc/nerve contact markers are transported with the pose, not recalculated as exercise-induced nerve compression. External weight is mass × 9.81 N/kg; floor-pose loads remain schematic. No safe lifting limits or medical recommendations are inferred. Modeling background: [Dreischarf et al., 2016](https://pubmed.ncbi.nlm.nih.gov/26873281/).

Body rendering repair: complete hand/arm binding, reachable two-segment limb targets, elastic skin edge constraints, and measured sole/palm support offsets prevent long skin bridges and floor penetration in the squat, pull, cat–cow and push-up presets. These remain illustrative pose mechanics, not a validated tissue solver.
