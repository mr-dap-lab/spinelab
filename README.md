...
The overlay is the **FMA7163 skin surface from the same BodyParts3D dataset as the spine**, transformed with the identical axis conversion and uniform scale. It replaces the procedural dummy. The surface is clustered for browser performance; source URL, checksum and processing are in `public/anatomy/body-source.json`. Run `scripts/pack-body.py` after downloading its documented source to `/tmp/spinelab-skin.stl` to reproduce the derived mesh. The body GLB and its derivatives retain CC BY-SA 2.1 Japan.
