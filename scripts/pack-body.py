...
import struct,json,pathlib,hashlib,math
...
for face in struct.iter_unpack('<12fH',data[84:]):
 ...
 for j in range(3):
  ...
  if key not in cells: cells[key]=len(sums);sums.append([0.,0.,0.,0,0.,0.,0.])
  n=cells[key];a=sums[n];a[0]+=x;a[1]+=y;a[2]+=z;a[3]+=1;a[4]+=face[0];a[5]+=face[1];a[6]+=face[2];tri.append(n)
...
# Omit external genital detail in the educational overlay. Project only the
# anterior central groin into a smooth pelvic envelope; spine registration,
# limbs, overall dimensions and posterior anatomy are unchanged.
def smooth(a,b,x):
 t=max(0,min(1,(x-a)/(b-a)));return t*t*(3-2*t)
anonymous=[]
normals=[]
for i,(x,y,z) in enumerate(verts):
 weight=(1-smooth(1.8,2.8,abs(x)))*smooth(-20,-18.5,y)*(1-smooth(-13,-11.8,y))
 envelope=-3.2-.18*(y+14)+.06*x*x
 if z<envelope:z+=(envelope-z)*weight
 anonymous.append((x,y,z))
 a=sums[i];normal=[-a[4],a[6],a[5]];length=math.sqrt(sum(v*v for v in normal)) or 1
 normal=[v/length for v in normal]
 if weight>0 and z<=envelope+.001:
  target=[.12*x,-.18,-1];length=math.sqrt(sum(v*v for v in target));target=[v/length for v in target]
  normal=[normal[j]*(1-weight)+target[j]*weight for j in range(3)]
 normals.extend(normal)
verts=anonymous
positions=struct.pack('<'+'f'*len(verts)*3,*[v for p in verts for v in p]);ib=struct.pack('<'+'I'*len(indices),*indices)
nb=struct.pack('<'+'f'*len(normals),*normals)
meta={'source':'https://raw.githubusercontent.com/Kevin-Mattheus-Moerman/BodyParts3D/main/assets/BodyParts3D_data/stl/FMA7163.stl','sha256':hashlib.sha256(data).hexdigest(),'license':'CC BY-SA 2.1 Japan','vertices':len(verts),'triangles':len(indices)//3,'processing':'Vertex clustering 8 mm, 4 mm head; same uniform coordinate transform as spine.glb. No independent body scaling. External genital surface smoothed into a neutral pelvic envelope.'}
g={'asset':{'version':'2.0','copyright':'BodyParts3D © The Database Center for Life Science; CC BY-SA 2.1 Japan; STL conversion Kevin Mattheus Moerman'},'scene':0,'scenes':[{'nodes':[0]}],'nodes':[{'mesh':0,'name':'FMA7163 skin'}],'meshes':[{'primitives':[{'attributes':{'POSITION':0,'NORMAL':2},'indices':1}]}],'buffers':[{'byteLength':len(positions)+len(ib)+len(nb)}],'bufferViews':[{'buffer':0,'byteOffset':0,'byteLength':len(positions),'target':34962},{'buffer':0,'byteOffset':len(positions),'byteLength':len(ib),'target':34963},{'buffer':0,'byteOffset':len(positions)+len(ib),'byteLength':len(nb),'target':34962}],'accessors':[{'bufferView':0,'componentType':5126,'count':len(verts),'type':'VEC3','min':[min(v[i] for v in verts) for i in range(3)],'max':[max(v[i] for v in verts) for i in range(3)]},{'bufferView':1,'componentType':5125,'count':len(indices),'type':'SCALAR'},{'bufferView':2,'componentType':5126,'count':len(verts),'type':'VEC3'}]}
js=json.dumps(g,separators=(',',':')).encode();js+=b' '*((-len(js))%4);binary=positions+ib+nb
