...
import { buildDiscTissues, deformPoint, buildNucleusExtrusion, tearAt } from './disc-tissue.js';
...
function deformDisc(part, scenario) {
  ...
  if ((scenario.rupture || 0) >= 35) {
    const original = part.geometry.attributes.position, sourceIndex = part.geometry.index, indices = [];
    for (let i = 0; i < (sourceIndex ? sourceIndex.count : original.count); i += 3) {
      const ids = [0, 1, 2].map(j => sourceIndex ? sourceIndex.getX(i + j) : i + j);
      const x = ids.reduce((sum, id) => sum + original.getX(id), 0) / 3 - c.x;
      const z = ids.reduce((sum, id) => sum + original.getZ(id), 0) / 3 - c.z;
      const radial = Math.hypot(x / (size.x / 2), z / (size.z / 2));
      if (radial > 0.55 && tearAt(scenario, Math.atan2(x / (size.x / 2), z / (size.z / 2)))) continue;
      indices.push(...ids);
    }
...
export function buildSpine(
  ...
  for (const part of discParts) {
    ...
    if (
      ...
      (!scenario.bulge && !scenario.compression && !scenario.rupture)
    )
      continue;
  ...
  for (const part of discParts) {
    if (focus && part.label !== level) continue;
    const geometry = buildNucleusExtrusion(constrainedParts.get(part.label) || part, scenarios[part.label] || NEUTRAL_SCENARIO);
    if (!geometry) continue;
    const mesh = new THREE.Mesh(geometry, collisionMaterial);
    mesh.position.y = shiftAbove(part.index + 1) - shrink[part.index] / 2;
    mesh.updateMatrixWorld(); colliders.push(mesh);
  }
  ...
  for (const part of parts) {
    if (part.kind === 'bone') {
      ...
      const scenario = scenarios[part.label] || NEUTRAL_SCENARIO,
        geometry =
          scenario.bulge === 0 && scenario.compression === 0 && !scenario.rupture
            ? part.geometry
            : deformDisc(constrainedParts.get(part.label) || part, scenario);
      const mesh = addMesh(
        group,
      ...
      pickables.push(mesh);
      const extrusionGeometry = buildNucleusExtrusion(constrainedParts.get(part.label) || part, scenario);
      if (extrusionGeometry) {
        const extrusion = addMesh(group, extrusionGeometry, new THREE.MeshPhysicalMaterial({
          color: '#657fac', roughness: 0.25, clearcoat: 0.8, side: THREE.DoubleSide,
        }), mesh.position.y);
        extrusion.userData.level = part.label;
        extrusion.userData.tissue = 'extruded-nucleus';
        pickables.push(extrusion);
      }
