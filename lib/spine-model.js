  ...
  for (const part of parts) {
    if (part.kind === 'bone') {
      ...
      if (tissueSection && part.label === level) {
        ...
        const normal = new THREE.Vector3(0, -1, part.slope);
        const constant =
          part.center.y - part.slope * part.center.z + mesh.position.y;
        mesh.material.clippingPlanes = [
          new THREE.Plane(normal, constant).normalize(),
        ];
        mesh.material.clipShadows = true;
        mesh.material.side = THREE.DoubleSide;
        const tissues = buildDiscTissues(part, scenario, mesh.position.y);
        group.add(tissues);
        tissues.traverse((o) => {
