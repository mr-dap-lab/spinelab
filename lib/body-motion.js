...
export function createBodyMotion(model,parts,sourceSkin) {
  ...
  const plantedHandRotation=new T.Quaternion().setFromAxisAngle(new T.Vector3(1,0,0),rad(90)).multiply(new T.Quaternion().setFromAxisAngle(new T.Vector3(0,1,0),Math.PI));
  const plantedHandShear=new T.Matrix4().set(1,0,0,0,0,1,0,0,0,-.3,1,0,0,0,0,1);
  const plantedFootRotation=new T.Quaternion().setFromAxisAngle(new T.Vector3(1,0,0),rad(-65));
  const supports=limbs.map((l,i)=>{
    const pivot=new T.Vector3(l.ankle.x,-45.2,-3);
    const toe=new T.Matrix4().makeTranslation(...pivot).multiply(new T.Matrix4().makeRotationX(rad(65))).multiply(new T.Matrix4().makeTranslation(...pivot.clone().negate()));
    let handBottom=Infinity,footBottom=Infinity;
    for(let v=0;v<bindings.length;v++){
      const b=bindings[v];if(!b||b.side!==i)continue;
      const p=new T.Vector3().fromArray(base,v*3);
      if(b.hand===1)handBottom=Math.min(handBottom,p.clone().sub(l.wrist).applyMatrix4(plantedHandShear).applyQuaternion(plantedHandRotation).y);
      if(b.foot===1)footBottom=Math.min(footBottom,p.clone().lerp(p.clone().applyMatrix4(toe),T.MathUtils.smoothstep(-p.z,2.5,3.8)).sub(l.ankle).applyQuaternion(plantedFootRotation).y);
    }
    return {handBottom:Number.isFinite(handBottom)?handBottom:-.8,footBottom:Number.isFinite(footBottom)?footBottom:-2.4};
  });
  ...
  function update(config,phase,dt=1/30,instant=false){
    ...
    const armReach=limbs[0].shoulder.distanceTo(limbs[0].elbow)+limbs[0].elbow.distanceTo(limbs[0].wrist)-.01;
    const legReach=limbs[0].hip.distanceTo(limbs[0].knee)+limbs[0].knee.distanceTo(limbs[0].ankle)-.08;
    const bodyReach=legReach+limbs[0].hip.distanceTo(limbs[0].shoulder);
    if(pose.floor==='kneeling'){
      const hip=limbs[0].hip.clone().applyMatrix4(dynamics.matrixAt(limbs[0].hip.y));
      const shoulder=limbs[0].shoulder.clone().applyMatrix4(dynamics.matrixAt(limbs[0].shoulder.y));
      const delta=shoulder.sub(hip),target=armReach-supports[0].handBottom-1.7-limbs[0].hip.distanceTo(limbs[0].knee);
      pose.hinge-=T.MathUtils.radToDeg(Math.asin(clamp(target/Math.hypot(delta.y,delta.z),-.9,.9))-Math.atan2(delta.y,-delta.z));
      dynamics.step(pose,config.load,0,false);
    }else if(pose.floor==='plank'){
      const rise=armReach*(1-.58*pose.pushDepth)-supports[0].handBottom+supports[0].footBottom;
      pose.hinge=T.MathUtils.radToDeg(Math.acos(clamp(rise/bodyReach,-1,1)));
      dynamics.step(pose,config.load,0,false);
    }
    ...
    let elevation=0,travel=0;
    if(pose.floor==='kneeling')elevation=floorY+1.7+limbs[0].hip.distanceTo(limbs[0].knee)-Math.max(...hips.map(p=>p.y));
    if(pose.floor==='plank'){
      elevation=floorY-supports[0].handBottom+armReach*(1-.58*pose.pushDepth)-shoulders[0].y;
      const ankleY=floorY-elevation-supports[0].footBottom;
      travel=28-hips[0].z-Math.sqrt(Math.max(.01,legReach*legReach-(ankleY-hips[0].y)**2));
    }
    const limbFrames=limbs.map((l,i)=>{
      const hip=hips[i],shoulder=shoulders[i];
      // Tucked toes: flex at the ankle in the sagittal plane. Turning the
      // feet around the vertical axis incorrectly laid the soles backwards.
      const footRotation=new T.Quaternion().setFromAxisAngle(new T.Vector3(1,0,0),floorPose?rad(-65):0);
      const toePivot=new T.Vector3(l.ankle.x,-45.2,-3);
      const toe=new T.Matrix4().makeTranslation(...toePivot).multiply(new T.Matrix4().makeRotationX(floorPose?rad(65):0)).multiply(new T.Matrix4().makeTranslation(...toePivot.clone().negate()));
      const handRotation=plantedHandRotation;
      ...
      for(let v=0;v<bindings.length;v++){
        ...
        if(bind.foot===1){p.fromArray(base,v*3);const t=T.MathUtils.smoothstep(-p.z,2.5,3.8);p.lerp(p.clone().applyMatrix4(toe),t).sub(l.ankle).applyQuaternion(footRotation);footBottom=Math.min(footBottom,p.y);}
        if(bind.hand===1){p.fromArray(base,v*3).sub(l.wrist).applyMatrix4(plantedHandShear).applyQuaternion(handRotation);handBottom=Math.min(handBottom,p.y);}
      }
      ...
      if(floorPose){
        let handZ=shoulder.z;
        if(pose.floor==='plank'){
          const topRise=armReach-handBottom+footBottom;
          handZ=28-Math.sqrt(bodyReach*bodyReach-topRise*topRise)-travel;
        }
        hand.set(shoulder.x,floorY-elevation-handBottom,handZ);
      }
      ...
      const palm=new T.Matrix4().makeTranslation(...hand).multiply(new T.Matrix4().makeRotationFromQuaternion(handRotation)).multiply(plantedHandShear).multiply(new T.Matrix4().makeTranslation(...l.wrist.clone().negate()));
      const elbowFlexion=180-T.MathUtils.radToDeg(shoulder.clone().sub(elbow).angleTo(hand.clone().sub(elbow)));
      return {elbowFlexion,foot,toe,palm,leg:[segmentMatrix(l.hip,l.knee,hip,knee),segmentMatrix(l.knee,l.ankle,knee,ankle)],arm:[segmentMatrix(l.shoulder,l.elbow,shoulder,elbow),segmentMatrix(l.elbow,l.wrist,elbow,hand)]};
    });
    ...
    for(let i=0;i<a.count;i++){
      ...
      if(bind){const [m1,m2]=limbFrames[bind.side][bind.limb];pb.copy(p).applyMatrix4(m1).lerp(p.clone().applyMatrix4(m2),bind.joint);if(bind.foot){const footPoint=p.clone().lerp(p.clone().applyMatrix4(limbFrames[bind.side].toe),T.MathUtils.smoothstep(-p.z,2.5,3.8));pb.lerp(footPoint.applyMatrix4(limbFrames[bind.side].foot),bind.foot);}if(floorPose&&bind.hand)pb.lerp(p.clone().applyMatrix4(limbFrames[bind.side].palm),bind.hand);pa.lerp(pb,bind.mix);
        ...
        if(bind.foot){const footNormal=nn.clone().lerp(nn.clone().transformDirection(limbFrames[bind.side].toe),T.MathUtils.smoothstep(-p.z,2.5,3.8));limbNormal.lerp(footNormal.transformDirection(limbFrames[bind.side].foot),bind.foot);}
    ...
    group.position.set(0,elevation,travel);for(const r of records){r.o.position.y+=elevation;r.o.position.z+=travel;}
    ...
    floor.position.set(0,floorY-elevation,-travel);
    metrics={elbowFlexion:Math.max(...limbFrames.map(f=>f.elbowFlexion)),maxJoint:Math.max(...dynamics.joints.flatMap(j=>j.q.map(v=>Math.abs(v)*180/Math.PI))),moment:Math.max(...dynamics.joints.map(j=>Math.abs(j.moment))),shortening:dynamics.joints.reduce((n,j)=>n+j.compression,0)/.04};
    return true;
  }
