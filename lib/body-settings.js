...
export const DEFAULT_BODY = {
  enabled: false, cycleTick: 0, opacity: 22, movement: 'manual', phase: 0, playing: false,
  speed: 1, hinge: 0, flexion: 0, side: 0, twist: 0, load: 0, drag: 'hinge',
};
...
export function movementPose(config, phase = config.phase) {
  ...
  switch (config.movement) {
    ...
    case 'squat': p.hinge = 28 * u; p.drop = 0.34 * u; p.retreat = 0.12 * u; p.arms = 0.65 * u; break;
    case 'catcow': p.hinge = 90; p.flexion = 25 * wave; p.floor = 'kneeling'; break;
    case 'lunge': p.drop = 0.23 * u; p.stride = 0.38 * u; p.hinge = 8 * u; break;
    case 'push': p.hinge = 82; p.drop = 0.16 * u; p.floor = 'plank'; break;
